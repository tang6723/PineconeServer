...
import com.ft.db.mapping.DeferredProfitsAccount;
 ...
 * @author Tang
 */
public class DeferredProfitsAccountDao {
     ...
     * @return 
     */
    public boolean insertItem(DeferredProfitsAccount item){
        String sql="INSERT  INTO  DeferredProfitsAccount (DeferredProfitsAccountID,BusinessTime,RelevantID,RelevantDesc,MembersID, "
        ...
        String[] parm={
            item.getDeferredProfitsAccountID(),
     ...
     * @return 
     */
    public boolean updateItem(DeferredProfitsAccount item){
        String sql="UPDATE  DeferredProfitsAccount "
                + "SET  BusinessTime=?, RelevantID=?, RelevantDesc=? ,MembersID=?,"
                + "MerchantID=?, AgentID=?, AccountCode=?, TopSubjects=?, TwoSubjects=?,"
                + "RecordTime=?, DebitCreditor=?, Amount=?, Summary=?, AuditTime=?,"
                + "AuditorID=?, Auditor=?,  Assessment=?, DeleteFlag=?, Remarks=? "
                + "WHERE DeferredProfitsAccountID=?";
        int affectRowCount = 0;
        String[] parm={            
            DatabaseUtility.getDate(item.getBusinessTime()),           
            item.getRelevantID(),
     ...
     * @return 
     */
    public boolean deleteItem(DeferredProfitsAccount item){
        String sql="UPDATE  DeferredProfitsAccount SET  DeleteFlag=?  WHERE DeferredProfitsAccountID=?";
        int affectRowCount = 0;
     ...
     * @return 
     */
    public List<DeferredProfitsAccount> getList(String strWhereQuery,String[] parm){
        String sql="SELECT * FROM DeferredProfitsAccount WHERE 1=1 "+strWhereQuery;        
        ResultSet rs ;
        
        List<DeferredProfitsAccount> itemList=new ArrayList();
        try{
            rs=DatabaseUtility.queryResultSet(sql, parm);
            while(rs.next()){
                DeferredProfitsAccount item=new DeferredProfitsAccount();
                item.setDeferredProfitsAccountID(rs.getString("DeferredProfitsAccountID"));
     ...
     * @return 
     */
    public DeferredProfitsAccount getItem(String strID){
        String sql="SELECT * FROM DeferredProfitsAccount WHERE DeferredProfitsAccountID=?";        
        ResultSet rs ;
        String[] parm={                        
            strID
        };
